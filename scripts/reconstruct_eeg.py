...
if __name__ == "__main__":
    ...
    model = VQ_Align(encoder_conf, decoder_conf).to(device)
    ...
    # Or: pick a particular channel/time.
    # Suppose you want standard_1020.index("OZ") and time window 3:
    # want_chan_idx = standard_1020.index("OZ")
    # idx = pick_token_indices(chans_valid, times_valid, want_chan_idx=want_chan_idx, want_time=3)
    # if idx is not None:
    #     plot_token_overlay(X_valid[idx], Xrec_valid[idx], title="OZ, window 3")
    ...
    # plt.legend()
    # plt.tight_layout()
    # plt.savefig("reconstruction_overlay.png", dpi=300)
    # plt.show()